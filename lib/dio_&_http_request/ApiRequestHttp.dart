...
class ApiRequestHttp {
  ...
  Future<dynamic> get({
    ...
    Function(dynamic error, dynamic val, String startTime, String endTime)?
        onError,
  }) async {
    try {
      ...
      dynamic response = await http
          .get(finalUrl, headers: headers as Map<String, String>)
      ...
      if (response.statusCode == 200) {
        ...
        switch (response.statusCode) {
          case 400:
            onError?.call(
                'Bad  request', response.statusCode, apiStartTime, apiEndTime);
            return null;
          case 401:
            onError?.call(
                'Unauthorized', response.statusCode, apiStartTime, apiEndTime);
            return null;
          case 403:
            onError?.call(
                'Forbidden', response.statusCode, apiStartTime, apiEndTime);
            return null;
          case 404:
            onError?.call(
                'Not found', response.statusCode, apiStartTime, apiEndTime);
            return null;
          case 500:
      ...
      onError?.call(
          'Uncaught exception', e.toString(), apiStartTime, apiEndTime);
      return null;
    }
    return null;
  }
  ...
  Future<dynamic> post({
    ...
    try {
      ...
      apiStartTime = DateTime.now().millisecondsSinceEpoch.toString();
      dynamic response = await http
          .post(finalUrl, body: data, headers: headers as Map<String, String>)
          .timeout(Duration(milliseconds: apiTimeOut));
      apiEndTime = DateTime.now().millisecondsSinceEpoch.toString();
  ...
  Future<dynamic> delete({
    ...
    try {
      ...
      apiStartTime = DateTime.now().millisecondsSinceEpoch.toString();
      dynamic response = await http
          .delete(finalUrl, headers: headers as Map<String, String>)
          .timeout(Duration(milliseconds: apiTimeOut));
      apiEndTime = DateTime.now().millisecondsSinceEpoch.toString();
      if (response.statusCode == 200) {
        onSuccess?.call(response, apiStartTime, apiEndTime);
  ...
  Future<dynamic> put({
    ...
    try {
      ...
      apiStartTime = DateTime.now().millisecondsSinceEpoch.toString();
      dynamic response = await http
          .put(finalUrl, body: data, headers: headers as Map<String, String>)
          .timeout(Duration(milliseconds: apiTimeOut));
      apiEndTime = DateTime.now().millisecondsSinceEpoch.toString();
      if (response.statusCode == 200 || response.statusCode == 201) {
        onSuccess?.call(response, apiStartTime, apiEndTime);
