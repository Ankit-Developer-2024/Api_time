...
import 'package:api_time/feature/view/widgets/appliction_info_for_user.dart';
...
class HomePageController extends GetxController {
  ...
  TextEditingController apiCallCountController =
      TextEditingController(text: '1');
  TextEditingController apiIntervalController =
  ...
  void onInit() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Get.size.width < 700 ? null : applictionInfoForUser();
    });
  }
  ...
  void setDataByUrl() {
    ...
    if (body.isNotEmpty) {
      try {
        ...
        CommonUiComponent().snackBar(
            firstTitle: 'Body Must be { key:value } and key Must be String ',
            secondTitle: e.toString(),
    ...
    if (apiRequestType.value == 'POST') {
      if (apiBodyType.value.isEmpty) {
        CommonUiComponent().snackBar(
            firstTitle: 'API Body Type is required',
            secondTitle: '',
            durationMilliseconds: 2000);
        return;
      }
    ...
    if (queryParamsError.value == false &&
      ...
      urls?.add({
        ...
        'data': jsonBody,
        'bodyType': apiBodyType.value,
        'requestType': apiRequestType.value,
        'headers': headers,
        "extractData": extractData.value
      });
      Get.back();
      urlController.text = '';
  ...
  Map<String, dynamic> parseQueryParams(String queryParamsString) {
    try {
      ...
      final queryParams = queryParamsString
          .split('\n')
          .map((item) => item.split('='))
      ...
      CommonUiComponent().snackBar(
          firstTitle: 'Param Must be key=value',
          secondTitle: e.toString(),
          maxWidth: Get.width / 2,
          durationMilliseconds: 2000);
      return {};
  ...
  Map<String, String> parseHeaders(String headersString) {
    try {
      ...
      CommonUiComponent().snackBar(
          firstTitle: 'Headers Must key: value',
          secondTitle: e.toString(),
          maxWidth: Get.width / 2,
          durationMilliseconds: 2000);
      return {};
  ...
  void startCallingApi() async {
    ...
    for (int j = 0; j < apiCallCount.value; j++) {
      for (int i = 0; i < urls!.length; i++) {
        apiMethod.value == 'Dio'
            ? apiCallDio(i, index, j)
            : apiCallHttp(i, index, j);
        index++;
      }
    ...
    switch (apiData["requestType"]) {
      case "GET":
        try {
          ...
          ApiRequestDio(
                  url: apiData["url"],
                  queryParams: apiData["queryParameter"],
                  apiTimeOut: apiTimeOut.value,
                  headers: apiData["headers"],
                  apiStartTime: apiStartTime.value,
                  apiEndTime: apiEndTime.value)
              .get(
            onSuccess: (res, apiStartTime, apiEndTime) {
              totalDuration.value =
                  int.parse(apiEndTime) - int.parse(apiStartTime);
            ...
            onError: (err, val, apiStartTime, apiEndTime) {
              ...
              apiDetails?[index] = {
                ...
                "duration": totalDuration.value,
                "resBody": responseString,
                "statusCode": err == "Response error"
                    ? val.statusCode.toString()
                    : err.toString(),
              };
          ...
          CommonUiComponent()
              .snackBar(firstTitle: e.toString(), secondTitle: '');
        }
      ...
      case "POST":
        try {
          ...
          dynamic finalData = apiData['extractData'] == 'cURL'
              ? apiData["data"]
              : apiData['bodyType'] == 'JSON'
                  ? apiData["data"]
                  : dio.FormData.fromMap(apiData["data"]);
          finalData = apiData['extractData'] == 'cURL'
              ? apiData['bodyType'] == 'JSON'
                  ? finalData
                  : dio.FormData.fromMap(apiData["data"])
              : finalData;
      ...
      default:
        CommonUiComponent()
            .snackBar(firstTitle: 'Invalid request type', secondTitle: '');
        break;
    }
  }
  ...
  void callApiFromCurl(String curlCommand) async {
    try {
      ...
      for (var curlCommand in l) {
        ...
        if (match != null) {
          requestType = match.group(1)?.toUpperCase() ??
              match.group(2)?.toUpperCase() ??
              '';
          url = match.group(3);
        } else {
          CommonUiComponent().snackBar(
              firstTitle: "No URL found in",
              secondTitle: curlCommand.toString());
          urls?.clear();
          apiDetails?.clear();
          successRequest.value = 0;
          failedRequest.value = 0;
        ...
        if (data != null) {
          if (match.group(1) == '--form' || match.group(1) == '-F') {
            ...
            if (keyValue.length == 2) {
              ...
              dataMap[key] = value
                  .replaceAll('"', '')
                  .replaceAll("'", ''); // Remove enclosing quotes
            }
          }
        }
      }
      ...
      try {
        RegExp dataRegex = RegExp(
            r"""--data\s+(["\'][\s\S]+[^"\']*[\s\S]+["\'])|-d\s+(["\'][\s\S]+[^"\']*[\s\S]+["\'])""");
        data = dataRegex.allMatches(curlCommand).first.group(1) ??
            dataRegex.allMatches(curlCommand).first.group(2);
      } catch (e) {
        if (kDebugMode) {
          print(e);
        }
      ...
      CommonUiComponent().snackBar(
          firstTitle: 'Please give valid Curl ',
          secondTitle: curlCommand,
          maxWidth: Get.width / 2,
    ...
    switch (apiData["requestType"]) {
      ...
      case "POST":
        try {
          ...
          finalData = finalData.runtimeType == dio.FormData
              ? Map.fromEntries(finalData.fields)
